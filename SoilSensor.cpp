...
bool isSoilDry()
{
  ...
  // Consider soil dry if moisture is below 50% (triggers RUNNING state)
  return (soilPercent < 50);
}

bool isSoilCritical()
{
  int soilPercent = ReadSoilSensor();
  // Consider soil critical if moisture is below 5% (triggers ERROR state)
  return (soilPercent < 5);
}